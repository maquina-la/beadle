...
if [[ "$RELEASE" == "1" && "$CODE_SIGN_IDENTITY" != "Developer ID Application:"* ]]; then
    echo "Release builds require CODE_SIGN_IDENTITY to name a Developer ID Application identity." >&2
    exit 1
fi
...
cp -f "$BUILT_EXECUTABLE" "$APP_BUNDLE/Contents/MacOS/$EXECUTABLE_NAME"
cp -f "packaging/Info.plist" "$APP_BUNDLE/Contents/Info.plist"
cp -f "packaging/AppIcon.icns" "$APP_BUNDLE/Contents/Resources/AppIcon.icns"
